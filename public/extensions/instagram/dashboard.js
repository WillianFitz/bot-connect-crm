...
/* ══════════════════════════════════════════════════════════
   INIT — carrega config.json se existir (extensão baixada pelo link do SaaS)
   ══════════════════════════════════════════════════════════ */
...
function initDashboard() {
  ...
  // Carrega configs salvas (ou já preenchidas pelo config.json)
  chrome.storage.local.get(
    ...
    (data) => {
      ...
      if (data.preConfigured) {
        ...
        if (tenantEl) { tenantEl.readOnly = true; tenantEl.title = "Pré-configurado para sua conta (não altere)."; }
      }
      renderProfilesTable(data.profiles || {});
    }
  );

  /* ── Salvar configuração (token e webhook obrigatórios) ── */
  document.getElementById("saveConfig").addEventListener("click", () => {
    ...
    if (!extensionToken || !webhookUrl) {
      setStatus("configStatus", "Preencha Token e URL do Webhook antes de salvar. Sem o token a extensão não envia leads.", false);
      return;
    }
    if (!tenantId) {
      setStatus("configStatus", "Tenant ID é obrigatório. Baixe a extensão pelo link da sua conta no painel.", false);
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  fetch(chrome.runtime.getURL("config.json"))
    ...
    .then(function(data) {
      if (data && data.tenantId != null && data.extensionToken && data.webhookUrl) {
        return new Promise(function(resolve) {
          chrome.storage.local.set({
            tenantId: String(data.tenantId),
            extensionToken: String(data.extensionToken),
    ...
    .catch(function() {})
    .finally(function() { initDashboard(); });
});
